...
type
  ...
  public
    ...
    //
    function GetTimeTableString(r: Integer): string;
...
function TExcelExporter.GetTimeTableString(r: Integer): string;
begin
  GetTimeList(r);
  SL.Insert(0,TimeListStartToken + '.R' + IntToStr(r));
  SL.Add(TimeListEndToken);
  result := SL.Text;
end;
...
begin
  ...
  SLToken.Add('Bib');
  cr := cl.Items[0];
  for i := 1 to cr.ITCount - 1 do
  begin
    SLToken.Add('IT' + IntToStr(i));
  end;
  SLToken.Add('FT');
  s := SLToken.DelimitedText;
  SL.Add(s);
